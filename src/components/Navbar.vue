      ...
      <template v-if="items.length === 0">
        <v-btn icon>
          <v-badge v-model="show" color="#D7BC2F" class="text--color" left>
            ...
            <v-icon @mouseover="show = true" @mouseout="show = false" @click="getSubtotal">mdi-cart</v-icon>
          </v-badge>
        </v-btn>
      </template>
      <v-menu v-model="menu" bottom left :close-on-content-click="false" v-if="items.length != 0">
        ...
        <v-list>
          ...
          <v-list-item>
            <v-btn block color="primary" link to="/checkout" @click="menu = false">Checkout</v-btn>
          </v-list-item>
        </v-list>
      </v-menu>
    </v-app-bar>
...
export default {
  ...
  methods: {
    ...
    logout() {
      localStorage.clear();
      Vue.axios.get("http://mitch.zerofood.cz/api/logout");
    ...
    checkIfIsLogged() {
      if (localStorage.getItem("id") == null) {
        ...
        Vue.axios
          .get(
            `http://mitch.zerofood.cz/api/user/${localStorage.getItem("id")}`
          )
          .then(response => {
