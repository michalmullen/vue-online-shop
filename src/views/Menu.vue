  ...
  <v-container class="mx-auto">
    <div v-for="card in cards" :key="card.day">
      <v-list-group value="false">
        ...
        <v-list-item-content>
          <v-layout row wrap>
            <v-flex xs12 md6 fluid class="pa-4" v-for="item in card.items" :key="item.title">
              <v-hover v-slot:default="{ hover }">
                <v-card>
                  ...
                  <v-card-text style="position: relative">
                    <v-btn absolute dark fab top right color="secondary" @click="basket(item.id)">
  ...
  methods: {
    basket($id) {
      ...
      if (localStorage.getItem("basket") == null) {
        Vue.axios
          .get(`http://mitch.zerofood.cz/item/${$id}`)
          .then(response => {
