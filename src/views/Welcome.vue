<template>
  <v-container class="fluid">
    <v-row class="text-center">
      <v-content>
        <h1 class="display-3">Order Food You Love With the Environment in Mind</h1>
        <p class="pt-4">
            ...
            <v-col cols="12" sm="8" md="8">
              <v-card class="elevation-12">
                <v-toolbar color="primary" dark flat>
                  <v-tabs v-model="tabs" background-color="primary" centered grow icons-and-text>
  ...
  methods: {
    login() {
      if (this.$refs.form.validate()) {
        Vue.axios
          .post("http://mitch.zerofood.cz/login", {
    ...
    register() {
      if (this.$refs.form.validate()) {
        Vue.axios
          .post("http://mitch.zerofood.cz/user", {
            name: this.name,
            email: this.email,
    ...
    setMenu() {
      Vue.axios
        .get("http://mitch.zerofood.cz/menu")
        .then(response => {
